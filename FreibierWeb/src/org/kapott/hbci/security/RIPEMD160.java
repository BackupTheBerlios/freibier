...
/*  $Id: RIPEMD160.java,v 1.1 2005/04/05 21:34:47 tbayen Exp $
     ...
     extends java.security.MessageDigestSpi
{
    ...
    protected byte[] engineDigest()
    {
        ...
        for (int i = 0; i < 4; i++) {
            hashValue[i] = (byte)((h0 >> (i << 3)) & 0xFF);
            hashValue[4 + i] = (byte)((h1 >> (i << 3)) & 0xFF);
            hashValue[8 + i] = (byte)((h2 >> (i << 3)) & 0xFF);
            hashValue[12 + i] = (byte)((h3 >> (i << 3)) & 0xFF);
            hashValue[16 + i] = (byte)((h4 >> (i << 3)) & 0xFF);
        }
    ...
    private void hashit()
    {
        ...
        for (byte j = 0; j < 80; j++) {
            T = roll(A + f(j, B, C, D) + X[r(j)] + K(j), s(j)) + E;
            A = E;
            E = D;
            D = roll(C, (byte)10);
            C = B;
            ...
            T = roll(A2 + f((byte)(79 - j), B2, C2, D2) + X[r2(j)] + K2(j), s2(j)) + E2;
            A2 = E2;
            E2 = D2;
            D2 = roll(C2, (byte)10);
            C2 = B2;
