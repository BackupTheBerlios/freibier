...
/*  $Id: HBCICallbackSwing.java,v 1.1 2005/04/05 21:34:48 tbayen Exp $
    ...
    extends HBCICallbackConsole
{
    ...
    public void callback(final HBCIPassport passport,int reason,String msg,int datatype,StringBuffer retData)
    {
        ...
        try {
            switch (reason) {
                ...
                case HAVE_INST_MSG:
                    showInstMessage(currentData,"instmsg");
                    break;
                ...
                default:
                    throw new HBCI_Exception(HBCIUtilsInternal.getLocMsg("EXCMSG_CALLB_UNKNOWN",reason+" ("+msg+")"));
            }
        } catch (Exception e) {
    ...
    private void ackInstKeys(final Hashtable currentData,final String winname)
    {
        ...
        SwingUtilities.invokeLater(new Runnable() { public void run() {
            try {
                ...
                String exp_st=HBCIUtils.data2hex(iniletter.getKeyExponent());
                String mod_st=HBCIUtils.data2hex(iniletter.getKeyModulus());
                String hash_st=HBCIUtils.data2hex(iniletter.getKeyHash());
                ...
                for (int i=0;i<8;i++) {
                    if (i==0)
                        constr.insets=new Insets(4,0,1,8);
                    else if (i==7)
                        constr.insets=new Insets(1,0,4,8);
                    else
                        constr.insets=new Insets(1,0,1,8);
                    
                    l=new JLabel(exp_st.substring(i*16*3,(i+1)*16*3-1));
                    l.setFont(font);
                ...
                for (int i=0;i<8;i++) {
                    ...
                    l=new JLabel(mod_st.substring(i*16*3,(i+1)*16*3-1));
                    l.setFont(font);
                    l.setForeground(Color.BLUE);
                    table.add(l,constr);
                    constr.gridy++;
                ...
                for (int i=0;i<2;i++) {
                    ...
                    l=new JLabel(hash_st.substring(i*10*3,(i+1)*10*3-1));
                    l.setFont(font);
    ...
    private void haveNewMyKeys(final Hashtable currentData,final String winname)
    {
        ...
        SwingUtilities.invokeLater(new Runnable() { public void run() {
            try {
                ...
                constr.gridx++;
                table.add(new JLabel(HBCIUtils.date2String(date)),constr);
                
                constr.gridx=0;constr.gridy++;
                table.add(new JLabel(HBCIUtilsInternal.getLocMsg("TIME")),constr);
                constr.gridx++;
                table.add(new JLabel(HBCIUtils.time2String(date)),constr);
    ...
    private void needSIZEntrySelect(final Hashtable currentData,final String winname)
    {
        ...
        SwingUtilities.invokeLater(new Runnable() { public void run() {
            ...
            String[][] tableData=(String[][])data.toArray(new String[0][]);
