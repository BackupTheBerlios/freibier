...
/*  $Id: GVWPDepotList.java,v 1.1 2005/04/05 21:34:47 tbayen Exp $
    ...
    extends HBCIJobImpl
{
    ...
    {
        super(getLowlevelName(),passport,new GVRWPDepotList());
    ...
    {
        // *** es muessen noch die antwortdaten eines 571 geparst werden
        buffer.append(Swift.decodeUmlauts(result.getProperty(header+".data535")));
        String data=buffer.toString();

        while (data.length()!=0) {
            try {
                String onerecord=Swift.getOneBlock(data);
                ...
                if (st_time!=null) {
                    entry.timestamp=new SimpleDateFormat("yyyyMMdd hhmmss").parse(st_date+" "+st_time);
                } else {
                    entry.timestamp=new SimpleDateFormat("yyyyMMdd").parse(st_date);
                }
                ...
                while (true) {
                    ...
                    if (st.substring(1,5).equals("HOLP")) {
                        ...
                        if (st.charAt(pos1)=='N') 
                            pos1++;
                        entry.total=new Value();
                        entry.total.curr=st.substring(pos1,pos1+3);
                        entry.total.value=HBCIUtils.string2Value(st.substring(pos1+3).replace(',','.'));
                        if (pos1>7)
                            entry.total.value*=-1;
                        break;
                    }
                ...
                if (st.substring(st.indexOf("//")+2).equals("Y")) {
                    ...
                    while (true) {
                        ...
                        st=Swift.getTagValue(onegattung,"90",new String[] {"A","B"},0);
                        if (st!=null) {
                            gattung.price=new Value();
                            ...
                            if (st.substring(7,11).equals("PRCT")) {
                                ...
                                gattung.price.curr=st.substring(12,15);
                                next=15;
                            }
                            gattung.price.value=HBCIUtils.string2Value(st.substring(next).replace(',','.'));
                        }
                        ...
                        if (st_date!=null) {
                            if (st_time!=null) {
                                gattung.timestamp_price=new SimpleDateFormat("yyyyMMdd hhmmss").parse(st_date+" "+st_time);
                            } else {
                                gattung.timestamp_price=new SimpleDateFormat("yyyyMMdd").parse(st_date);
                            }
                        }
                        
                        st=Swift.getTagValue(onegattung,"93B",0);
                        String st_type=st.substring(7,11);
                        gattung.saldo=new Value();
                        ...
                        pos1=12;
                        if (st.charAt(pos1)=='N')
                            gattung.saldo.value=-1*HBCIUtils.string2Value(st.substring(pos1+1).replace(',','.'));
                        else
                            gattung.saldo.value=HBCIUtils.string2Value(st.substring(pos1).replace(',','.'));
                        ...
                        while (true) {
                            ...
                            if (st.substring(1,5).equals("HOLD")) {
                                ...
                                if (st.charAt(pos1)=='N') 
                                    pos1++;
                                gattung.depotwert=new Value();
                                gattung.depotwert.curr=st.substring(pos1,pos1+3);
                                gattung.depotwert.value=HBCIUtils.string2Value(st.substring(pos1+3).replace(',','.'));
                                if (pos1>7)
                                    gattung.depotwert.value*=-1;
                                break;
                            }
                        }
                        ...
                        while (true) {
                            ...
                            if (st.substring(1,5).equals("ACRU")) {
                                ...
                                if (st.charAt(pos1)=='N') 
                                    pos1++;
                                gattung.stueckzinsbetrag=new Value();
                                gattung.stueckzinsbetrag.curr=st.substring(pos1,pos1+3);
                                gattung.stueckzinsbetrag.value=HBCIUtils.string2Value(st.substring(pos1+3).replace(',','.'));
                                if (pos1>7)
                                    gattung.stueckzinsbetrag.value*=-1;
                                break;
                            }
                        }
                        ...
                        if (st!=null) {
                            ...
                            gattung.xchg_kurs=HBCIUtils.string2Value(st.substring(15).replace(',','.'));
                        }
                        ...
                        if (st!=null) {
                            ...
                            gattung.curr=Swift.getLineFieldValue(formtext,"1",0);
                            gattung.wptype=Swift.getLineFieldValue(formtext,"1",1);
                            gattung.branche=Swift.getLineFieldValue(formtext,"1",2);
                            gattung.countryEmittent=Swift.getLineFieldValue(formtext,"1",3);
                            
                            st=Swift.getLineFieldValue(formtext,"1",4);
                            if (st!=null)
                                gattung.kauf=new SimpleDateFormat("yyyyMMdd").parse(st);
                            st=Swift.getLineFieldValue(formtext,"1",5);
                            if (st!=null)
                                gattung.faellig=new SimpleDateFormat("yyyyMMdd").parse(st);
                            
                            st=Swift.getLineFieldValue(formtext,"2",0);
                            if (st!=null) {
                                gattung.einstandspreis=new Value();
                                gattung.einstandspreis.value=HBCIUtils.string2Value(st.replace(',','.'));
                                gattung.einstandspreis.curr="%";
                            }
                            st=Swift.getLineFieldValue(formtext,"2",1);
                            if (st!=null)
                                gattung.einstandspreis.curr=st;
                            
                            st=Swift.getLineFieldValue(formtext,"2",2);
                            if (st!=null)
                                gattung.zinssatz=HBCIUtilsInternal.string2Double(st.replace(',','.'));
                        ...
                        while (true) {
                            ...
                            pos1=17;
                            if (st.charAt(pos1)=='N')
                                subsaldo.saldo.value=-1*HBCIUtils.string2Value(st.substring(pos1+1).replace(',','.'));
                            else
                                subsaldo.saldo.value=HBCIUtils.string2Value(st.substring(pos1).replace(',','.'));
                            ...
                            if (formtext!=null) {
                                ...
                                subsaldo.lager=Swift.getLineFieldValue(formtext,"2",1);
                                
                                st=Swift.getLineFieldValue(formtext,"2",2);
                                if (st!=null)
                                    subsaldo.lockeduntil=new SimpleDateFormat("yyyyMMdd").parse(st);
                                
                                subsaldo.comment=Swift.getLineFieldValue(formtext,"3",0);
                                st=Swift.getLineFieldValue(formtext,"4",0);
                                if (st!=null) {
