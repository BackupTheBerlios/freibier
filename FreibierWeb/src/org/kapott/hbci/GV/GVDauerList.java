...
/*  $Id: GVDauerList.java,v 1.1 2005/04/05 21:34:47 tbayen Exp $
    ...
    extends HBCIJobImpl
{
    ...
    {
        super(getLowlevelName(),passport,new GVRDauerList());
    ...
    protected void extractResults(Properties result,String header,int idx)
    {
        ...
        entry.value.value=HBCIUtils.string2Value(result.getProperty(header+".BTG.value"));
        entry.value.curr=result.getProperty(header+".BTG.curr");
        ...
        if ((st=result.getProperty(header+".date"))!=null)
            entry.nextdate=HBCIUtils.string2Date(st);

        entry.orderid=result.getProperty(header+".orderid");

        entry.firstdate=HBCIUtils.string2Date(result.getProperty(header+".DauerDetails.firstdate"));
        ...
        if ((st=result.getProperty(header+".DauerDetails.lastdate"))!=null)
            entry.lastdate=HBCIUtils.string2Date(st);
        ...
        if (entry.aus_available) {
            ...
            if ((st=result.getProperty(header+".Aussetzung.startdate"))!=null)
                entry.aus_start=HBCIUtils.string2Date(st);
            if ((st=result.getProperty(header+".Aussetzung.enddate"))!=null)
                entry.aus_end=HBCIUtils.string2Date(st);
            entry.aus_breakcount=result.getProperty(header+".Aussetzung.number");
            entry.aus_newvalue=result.getProperty(header+".Aussetzung.newvalue.value");
            entry.aus_newcurr=result.getProperty(header+".Aussetzung.newvalue.curr");
        }
