...
/*  $Id: AnalyzeReportOfTransactions.java,v 1.1 2005/04/05 21:34:48 tbayen Exp $
...
public final class AnalyzeReportOfTransactions 
{
        ...
        throws Exception
    {
        ...
        try {
            ...
            hbciHandle=new HBCIHandler((version.length()!=0)?version:"210",passport);
            ...
            if (result.isOK()) {
                ...
                int               numof_lines=lines.length;

                for (int j=0;j<numof_lines;j++) { // alle Umsatzeinträge durchlaufen
                    GVRKUms.UmsLine entry=lines[j];

                    // für jeden Eintrag ein Feld mit allen Verwendungszweckzeilen extrahieren
                    String[] usages=entry.usage;
                    int      numof_usagelines=usages.length;

                    for (int k=0;k<numof_usagelines;k++) { // alle Verwendungszweckzeilen durchlaufen
                        String usageline=usages[k];
                        ...
                        if (usageline.equals("Rechnung 12345")) { 
                            ...
                            // entry.cd enthält "C" für Haben- und "D" für Soll-Buchung
                            // entry.value enthält gebuchten Betrag (immer positiv)
