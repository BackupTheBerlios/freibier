...
/*  $Id: MultipleSyntaxElements.java,v 1.1 2005/04/05 21:34:48 tbayen Exp $
...
public abstract class MultipleSyntaxElements
{
    ...
    protected abstract SyntaxElement createNewElement(Node ref, String path, int idx, Document syntax);

    /** siehe SyntaxElement::parseElementList() */
    protected abstract SyntaxElement parseNewElement(Node ref, String path, char predelim, int idx, StringBuffer res, int fullResLen,Document syntax, Hashtable predefs,Hashtable valids);
    ...
    private void initData(Node ref, String path, Document syntax)
    {
        ...
        try {
            ...
            SyntaxElement child=createNewElement(ref, path, 0, syntax);
            if (child!=null)
            ...
            for (int i = 1; i < minnum; i++) {
                child=createNewElement(ref, path, i, syntax);
                if (child!=null)
                    child.setParent(this);
            }
    ...
    protected boolean propagateValue(String destPath, String value,boolean tryToCreate,boolean allowOverwrite)
    {
        ...
        if (tryToCreate) {
            ...
            if (destPath.startsWith(path) && 
                ...
                int idx=destPath.indexOf(".",pathLen);
                if (idx==-1)
                    idx=destPathLen;
                int number=Integer.parseInt(destPath.substring(pathLen+1,idx));

                if (number>elements.size()) {
                    String temppath=path.substring(0,path.lastIndexOf("."));

                    for (int i=elements.size();i<number;i++) {
                        SyntaxElement child=createNewElement(ref,temppath,i,syntax);
                        if (child!=null)
                            child.setParent(this);
                    }
                }
    ...
    protected String getValueOfDE(String path)
    {
        ...
        for (Iterator i = elements.listIterator(); i.hasNext(); ) {
            ...
            String temp = e.getValueOfDE(path);
            if (temp != null)
                ret = temp;
        }
    ...
    protected String getValueOfDE(String path, int zero)
    {
        ...
        for (Iterator i = elements.listIterator(); i.hasNext(); ) {
            ...
            String temp = e.getValueOfDE(path, zero);
            if (temp != null)
                ret = temp;
        }
    ...
    protected SyntaxElement getElement(String path)
    {
        ...
        for (Iterator i = elements.listIterator(); i.hasNext(); ) {
            SyntaxElement e = (SyntaxElement)(i.next());
            SyntaxElement temp = e.getElement(path);
            if (temp != null)
                ret = temp;
        }
        ...
        try {
            while (!ready) {
                ...
                try {
                    ...
                    SyntaxElement child=parseNewElement(ref,path, 
                ...
                if ((maxnum != 0 && idx >= maxnum) ||
                        (maxnum == 0 && (save.toString().equals(res.toString())))) {
                    ready = true;
                }
            }
        } catch (RuntimeException e) {
