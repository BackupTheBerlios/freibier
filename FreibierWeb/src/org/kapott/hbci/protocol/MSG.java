...
/*  $Id: MSG.java,v 1.1 2005/04/05 21:34:48 tbayen Exp $
    ...
    extends SyntaxElement
{
    ...
    private void propagateUserData(String name, Hashtable clientValues)
    {
        ...
        while (e.hasMoreElements()) {
            ...
            if (key.startsWith(name+".") && value.length()!=0) {
                if (!propagateValue(key,value,TRY_TO_CREATE,DONT_ALLOW_OVERWRITE)) {
                    HBCIUtils.log(HBCIUtilsInternal.getLocMsg("WRN_MSG_NOSUCHPATH",new Object[] {key,value}),HBCIUtils.LOG_WARN);
                }
            }
        }
    }
    ...
    public Properties getData()
    {
        ...
        for (Enumeration e = hash.keys(); e.hasMoreElements(); ) {
            String key = (String)(e.nextElement());
            p.setProperty(key.substring(getName().length()+1), (String)(hash.get(key)));
        }
